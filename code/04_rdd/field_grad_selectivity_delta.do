/**********************************************************************
* field_grad_selectivity_delta.do
*
* Objetivo:
*   Graficar relación entre:
*
*       y = delta graduation rate = target - next-best
*       x = delta selectivity     = target - next-best
*
*   Agrupado por field.
*
...
foreach v in field delta_selectivity delta_grad_target_8y {

    capture confirm variable `v'

    if _rc != 0 {
        di as error "Falta variable necesaria: `v'"
        di as error "Revisa nombres con: ds *field* *delta* *select* *grad*"
        exit 111
    }
}
...
keep if !missing(delta_selectivity, delta_grad_target_8y)

capture confirm variable has_nextbest
if _rc == 0 {
    keep if has_nextbest == 1
}
...
gen x_delta_selectivity = delta_selectivity
gen y_delta_grad_rate   = delta_grad_target_8y
...
sort field x_delta_selectivity
...
collapse ///
    (mean) delta_selectivity = x_delta_selectivity ///
    (mean) delta_grad_rate   = y_delta_grad_rate ///
    (count) n_bin            = y_delta_grad_rate, ///
    by(field bin)

encode field, gen(field_id)

sort field_id delta_selectivity
...
foreach f of local fieldids {
    ...
    * Puntos binned por field
    local plots `"`plots' (scatter delta_grad_rate delta_selectivity if field_id == `f', msymbol(O) msize(tiny))"'

    * Línea ajustada por field
    local plots `"`plots' (lfit delta_grad_rate delta_selectivity if field_id == `f', lwidth(medthin))"'
...
twoway `plots', ///
    ...
    ytitle("Delta graduation rate: target - next-best") ///
    title("Delta Graduation Rate and Delta Selectivity by Field") ///
    subtitle("Target vs next-best. `nbins' bins by field") ///
    legend(order(`legend_order') cols(2) size(vsmall)) ///
    graphregion(color(white)) ///
    plotregion(color(white))

graph export "$output/figures/field_relationships/delta_grad_rate_delta_selectivity_by_field.pdf", replace

/**********************************************************************
* 11. Guardar base colapsada para revisar
**********************************************************************/

save "$processed/field_grad_selectivity_delta_bins.dta", replace

di as text "=================================================="
di as result "Listo. Figura creada:"
di as result "$output/figures/field_relationships/delta_grad_rate_delta_selectivity_by_field.pdf"
di as result "Base de bins:"
di as result "$processed/field_grad_selectivity_delta_bins.dta"
di as text "=================================================="