/**********************************************************************
* 09_build_next_best_admitted_with_attributes.do
*
* Construye next-best admitido y pega el lookup único:
*   $processed/program_year_attributes_nextbest.dta
*
* Outputs:
*   $processed/next_best_admitted_all_attributes.dta
*   $processed/next_best_admitted_delta_attributes.dta
**********************************************************************/
...
if _rc != 0 {
    di as error "No existe `attr_lookup'. Corre primero 08b_build_program_year_attributes_nextbest.do"
    exit 601
}
        ...
        if _rc == 0 {
            ...
            if _rc == 0 {
                ...
                gen double delta_`v' = target_`v' - nextbest_`v' ///
                    if has_nextbest == 1 & !missing(target_`v', nextbest_`v')
...
* Mantener variables centrales
capture confirm variable score_rd
if _rc != 0 gen score_rd = application_score - cutoff_regular

capture confirm variable above_cutoff
if _rc != 0 gen above_cutoff = score_rd >= 0 if !missing(score_rd)
...
egen target_id = group(mrun ao_proceso target_preferencia target_codigo_carrera)
...
/**********************************************************************
* 3. Elegir primera preferencia inferior factible
**********************************************************************/
...
/********************************************************************************
* 4. Unir targets con next-best y pegar atributos
********************************************************************************/
...
************************************************************
* Crear versión corta del lookup para evitar nombres >32 caracteres
************************************************************
...
capture drop delta_selectivity_10 delta_p50_selectivity_10
...
di as text "=================================================="
di as result "Diagnóstico final next-best + atributos"
di as text "=================================================="
...
save "$processed/next_best_admitted_all_attributes.dta", replace

preserve
    keep if has_nextbest == 1
    save "$processed/next_best_admitted_delta_attributes.dta", replace
restore

di as result "Bases guardadas en:"
di as result "$processed/next_best_admitted_all_attributes.dta"
di as result "$processed/next_best_admitted_delta_attributes.dta"
