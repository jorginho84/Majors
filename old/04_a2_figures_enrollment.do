...
*---- Figures about enrollment (enrolls uni and enroll in target degree.)
clear all
*--- Globals -------------------------------------------------------------------
...
*reghdfe grad_target8 above_cutoff score_rd 1.above_cutoff#c.score_rd if abs(score_rd) <= 25 & (2022 - año_proceso >= 8) , absorb(i.año_proceso#i.t_codigo_carrera) cluster (i.año_proceso#i.t_codigo_carrera)

*reghdfe earningsTot10 above_cutoff score_rd 1.above_cutoff#c.score_rd if abs(score_rd) <= 25, absorb(i.año_proceso#i.t_codigo_carrera)
...
* Figure 1: rd_enroll_uni
reduced_form_plot enrolls_uni 1 score_rd above_cutoff ${b_bwr_opt} ${b_bwr_opt} 25 25 5 5 " " año_proceso t_codigo_carrera 9 score_rd2 "Enrolls_in_any_university_degree" all

foreach d in 1 2 3 4 5 6 {
* Figure 1 (deltas): rd_enroll_uni for deltas
reduced_form_plot enrolls_uni 1 score_rd above_cutoff ${b_bwr_opt} ${b_bwr_opt} 25 25 5 5 " " año_proceso t_codigo_carrera 9 score_rd2 "Enrolls_in_any_university_degree_delta`d'" `d'
}

* Figure 2: rd_enroll_target for deltas
reduced_form_plot new_enrolls_target 1 score_rd above_cutoff ${b_bwr_opt} ${b_bwr_opt} 25 25 5 5 " " año_proceso t_codigo_carrera 9 score_rd2 "New_enrolls_in_target_degree" all

foreach d in 1 2 3 4 5 6 {
* Figure 2 (deltas): rd_enroll_target for deltas
reduced_form_plot new_enrolls_target 1 score_rd above_cutoff ${b_bwr_opt} ${b_bwr_opt} 25 25 5 5 " " año_proceso t_codigo_carrera 9 score_rd2 "New_enrolls_in_target_degree_delta`d'" `d'
}
